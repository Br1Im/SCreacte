...
async def startup_event():
    print("🚀 Запуск сервера SCreate...")
    
    # Подключение статических файлов фронтенда
    frontend_dist_path = os.path.join(os.path.dirname(__file__), "..", "frontend", "dist")
    if os.path.exists(frontend_dist_path):
        app.mount("/", StaticFiles(directory=frontend_dist_path, html=True), name="static")
        print(f"✅ Фронтенд подключен из: {frontend_dist_path}")
    else:
        print(f"⚠️  Папка фронтенда не найдена: {frontend_dist_path}")
        print("💡 Запустите 'npm run build' в папке frontend для сборки")
...
async def generate_quest(request: QuestRequest):
    ...
    try:
        print(f"Получен запрос на генерацию квеста: {request.input_method}")
        ...
        if request.input_method == "file" and not request.file_content:
            raise HTTPException(status_code=400, detail="Необходимо загрузить файл")
        
        # Генерируем квест
        quest = create_mock_quest(request)
        
        print(f"Квест успешно сгенерирован: {quest.title}")
        return quest
